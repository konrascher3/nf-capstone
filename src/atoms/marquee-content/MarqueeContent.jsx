		...
		<>
			...
			{data &&
				<>
					...
					<Typography variant="caption">
						{`Total Mcap: ${new Intl.NumberFormat("en-US", {notation: "compact", style: 'currency', currency: 'USD'}).format(data["Market Cap"][0].toString())}`}
					</Typography>
					<Typography variant="caption">
						(<span style={data["Market Cap"][1] > 0 ? {color: "green"} : {color: "red"}}>{data["Market Cap"][1].toString()}%</span>)
					</Typography>
					<Typography variant="caption">
						<Box sx={{ ml: .75, mr: .75 }}>
							<Typography variant="caption" color="text.secondary">
					...
					<Typography variant="caption">
						{`24h Vol: ${new Intl.NumberFormat("en-US", {notation: "compact", style: 'currency', currency: 'USD'}).format(data["24h Vol"].toString())}`}
					</Typography>
					<Typography variant="caption">
						<Box sx={{ ml: .75, mr: .75 }}>
							<Typography variant="caption" color="text.secondary">
